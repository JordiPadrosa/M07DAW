...
if($_POST["method"] == "signup") {
    // Si un dels camps nom, correu o contrasenya esta buit redirecciona a index.php?signup_campsObligatoris_error
    if($_POST["nom"] == "" || $_POST["correu"] == "" || $_POST["contrasenya"] == ""){
        header("Location: index.php?signup_campsObligatoris_error", true, 303);
    }// Si el nom, correu o contrasenya són massa llargs retorna l'error corresponent
    elseif(strlen($_POST["nom"]) > 20){
        header("Location: index.php?signup_nom_llarg", true, 303);
    }elseif(strlen($_POST["correu"]) > 30){
        header("Location: index.php?signup_correu_llarg", true, 303);
    }elseif(strlen($_POST["contrasenya"]) > 32){
        header("Location: index.php?signup_contrasenya_llarga", true, 303);
    }else{
...
elseif($_POST["method"] == "signin"){
    ...
    }// Si el correu o contrasenya són massa llargs retorna l'error corresponent
    elseif(strlen($_POST["correu"]) > 30){
        header("Location: index.php?signin_correu_llarg", true, 303);
    }elseif(strlen($_POST["contrasenya"]) > 32){
        header("Location: index.php?signin_contrasenya_llarga", true, 303);
    }else{
 ...
 *
 * @param string $email que conte el correu
 * @return string | null
 */
